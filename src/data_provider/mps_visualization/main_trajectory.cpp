...
int main(int argc, char** argv) {
 ...
 TemporalEyeGazeData eyeGazeRecords;
 {
   // Read the camera trajectory if any
   {
     ...
     std::cout << "loading trajectory poses" << std::endl;
     loadedPoses = readBaseTrajectory(locationFilePath);
     /*
 ...
 std::cout << "Convert data for 3D visualization." << std::endl;
   ...
   // Convert read Eye Gaze data for 3D visualization
   {
     ...
     for (const auto& it : eyeGazeRecords) {
       ...
       auto pose_info = std::lower_bound(
           ...
           loadedPoses.end(),
           eyeGazeTimestamp,
           [](const TrajectoryPose_base& pose, std::chrono::nanoseconds value) {
             return pose.utcTimestamp < value;
           });
 ...
 pangolin::CreateWindowAndBind("AlphaRoboticsHomeAloneKitTrajectoryViewer", 1280, 800);
 glEnable(GL_DEPTH_TEST);

 // Define Projection and initial ModelView matrix
 pangolin::OpenGlRenderState s_cam(
     pangolin::ProjectionMatrix(1280, 800, 800, 800, 640, 480, 0.2, 100),
